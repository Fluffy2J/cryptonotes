...
\\
	Suppose there exists some IND-CPA adversary against ElGamal. Given only public information, this adversary can send us a pair of messages, we can ``encrypt" one under ElGamal, and it will tell us which one we have ``encrypted". This quote marks are necessary, since we won't be performaing standard ElGamal encryption. We will exploit this adversary by packaging up $\blue{g}^\red{x}$, $\blue{g}^\red{y}$ and $\blue{g}^\red{z}$ in our challenge, and using the results to solve the Decisional Diffie Hellman problem for us.\\
\\
