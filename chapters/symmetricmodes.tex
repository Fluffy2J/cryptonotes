...
\section{Block Cipher Primitives}
    ...
    \end{table}
    \\
    An \textbf{S-Box} is a mapping of bit-strings to other bit-strings, theses do not have to be the same length, but if they are not, the output is almost always of a shorter length. Table~\ref{fig:lookupsbox} shows an example of an S-Box which maps 4-bit strings to 4-bit strings.\\
    \\
    We want to make the substitution as \textbf{non-linear} as possible. Formally, it would be linear if Equation~\ref{equ:sboxlin} was true, or at least was true was large probability. Having a linear S-Box is bad, because if we want to give a bit-string to an oracle, but can't because it is a message of ciphertext, we can transform it and then perform the reverse of that transform of on the result.
    \begin{equation}
        \begin{array}{l}
        ...
        \label{equ:sboxlin}
    \end{equation}
    We can do this to the S-Box in Table~\ref{fig:lookupsbox}. The proof given in the slides is
    %\begin{align}
        %s_1=3,s_2=6

    %    \Delta s=s_1\oplus s_2=5
    %\end{align}
